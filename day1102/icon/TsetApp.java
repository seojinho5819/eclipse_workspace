...
public class TsetApp extends JFrame{
	Canvas can;
	JButton bt;
	JTextField t;
	...
	public TsetApp() {
		...
		//이럴땐, 즉 일회성의 객체 사용인 경우 내부익명 클래스로 사용 할 수 있다...
		//Inner Anonymous Class 내부틱명클래슬라 한다!!!
		//형식은 앞의 new로 선언된 클래스를 구현한 클래스로 이해하면 된다.
		//주로 이벤트를 수현한 클래스는 해당 어플리 케이션에서만 사용되므로, 재사용가능성이 
		//없다. 따라서 내부 익명클래스로 많이 사용된다..
		can = new Canvas() {
		...
		//내부 익명클래스는 지역변수가 final로 된것만 접근가능..
		final int x =0;
